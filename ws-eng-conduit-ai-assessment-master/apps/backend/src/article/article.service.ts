...
export class ArticleService {
  constructor(
    ...
    private readonly tagsRepository: EntityRepository<Tag>) {}
  ...
  async create(userId: number, dto: CreateArticleDto) {
    ...
    const tags_input = dto.tagList.toString().split(',');
    article.tagList.push(...tags_input);
    
    const createdTags = [];

    for (const name of tags_input) {
      let tag = await this.tagsRepository.findOne({ tag: name });
      if(!tag)
      {
        const tagDto = {tag : name}  ;
        tag = await this.tagsRepository.create(tagDto);
      }
  ...
  async update(userId: number, slug: string, articleData: any): Promise<IArticleRO> {
    ...
    articleData.tagList = articleData.tagList.toString().split(',');
